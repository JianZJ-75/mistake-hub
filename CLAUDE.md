...
- **分层**：Controller (Dto) -> Manager（可选，跨 Service 编排） -> Service -> Mapper (BaseMapper)。
...
### Manager 层（循环依赖处理）
- **禁止使用 `@Lazy` 打破循环依赖**。`@Lazy` 只是延迟代理注入，循环仍然存在，属于止痛药而非治根。
- 当多个 Service 出现循环依赖时，抽取 **Manager 层**（`manager/` 目录）做跨 Service 编排，让依赖图变成单向。
- **分层**：`Controller → Manager → Service → Mapper`。Controller 对需要跨 Service 协调的操作调用 Manager，其余仍直调 Service。
- **命名**：`{业务}Manager`，如 `MistakeManager`。放在 `manager/` 目录下。
- **注解**：`@Service` + `@Slf4j`（与 Service 一致）。
- **原则**：Manager 只做编排（调 A Service + 调 B Service），不放数据访问逻辑。事务注解 `@Transactional` 加在 Manager 方法上（编排层控制事务边界）。
- **示例**：`MistakeService` 和 `ReviewRecordService` 互相依赖 → 抽出 `MistakeManager.add()` 先调 `mistakeService.add()` 再调 `reviewRecordService.createRecord()`，两个 Service 不再互相引用。
...
### 枚举值使用规范
- **后端禁止硬编码枚举字符串**：代码中引用枚举值必须通过枚举常量（如 `Role.ADMIN.getCode()`、`TagType.SUBJECT.getCode()`），禁止直接写字面量 `"admin"`、`"SUBJECT"` 等。
- **跨实体共享的常量**放在 Entity 类上（`public static final`），如 `Mistake.STATUS_VALID = 1`，各 Service 统一引用。
- **每个枚举类型必须暴露 API 给前端**：在对应 Controller 中提供 `GET /{module}/types` 或 `GET /{module}/roles` 接口，返回 `List<BasicEnumPojo>`（调用枚举的 `listAll()` 方法）。
- **前端禁止硬编码枚举选项**：Select/Filter 的选项列表必须从后端枚举 API 拉取，页面 `useEffect` 中调用并存入 state，通过 `Map<code, displayNameCn>` 做显示转换。禁止在前端维护 `TYPE_LABEL` 等静态映射。
- **前端业务逻辑中的枚举 code 比较**（如 `=== "admin"` 判断是否管理员）属于合理使用，无需替换。
...
### 操作日志（`@OperationLogAnno`）
- **记录范围**：**管理员的写操作**加日志，**查询接口**和**用户自服务操作**不加。
- **需要记录**：管理员对他人数据或系统配置的增/删/改操作（如改角色、重置密码、管理标签、改系统参数、管理端编辑错题）。
- **不需要记录**：所有 GET 查询接口；用户对自己数据的操作（录入错题、复习答题、改自己昵称/密码、登录）。
- **注解属性使用枚举类型**：`action` 用 `OperationAction` 枚举，`targetType` 用 `OperationTargetType` 枚举，禁止传字符串。新增操作时同步在枚举中添加对应值。
